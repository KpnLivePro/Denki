...
class Admin(commands.Cog):
    ...
    @app_commands.command(
        name="config",
        description="View and manage Denki config for your server. Admin only.",
    )
    ...
    async def _config(self, ctx_or_interaction: Any, is_slash: bool) -> None:
        ...
        notif_channel = (
            f"<#{config['notif_channel']}>"
            if config.get("notif_channel")
            else "`Not set`"
        )
        notif_role = (
            f"<@&{config['notif_role']}>" if config.get("notif_role") else "`Not set`"
        )
        ...
        ai_status = (
            f"`{E_TEA_AI} Active — {seasons_left} season(s) left`"
            if seasons_left > 0
            else "`Not purchased`"
        )
        cashback_status = (
            f"`{E_POT} Active`" if config.get("cashback_enabled") else "`Not purchased`"
        )

        embed = UI.base(f"> {E_GEAR} *Denki config — **{guild.name}***")
        embed.add_field(
            name=f"{E_ANNOUNCE} Notif channel", value=notif_channel, inline=True
        )
        embed.add_field(name=f"{E_BELL} Notif role", value=notif_role, inline=True)
        embed.add_field(name=f"{E_SHOP} Shop", value=shop_status, inline=True)
        embed.add_field(name=f"{E_DAILY} Daily", value=daily_status, inline=True)
        embed.add_field(name=f"{E_WORK} Work", value=work_status, inline=True)
        embed.add_field(name=f"{E_ROB} Rob", value=rob_status, inline=True)
        embed.add_field(name=f"{E_TEA_AI} Tea AI", value=ai_status, inline=True)
        embed.add_field(
            name=f"{E_POT} Weekly Cashback", value=cashback_status, inline=True
        )
        embed.set_footer(
            text="Use /setnotifchannel, /setnofifrole, /earnsettings to update  •  Buy upgrades in /shop"
        )
    ...
    @app_commands.command(
        ...
        description="Set the channel for Denki announcements. Admin only.",
    )
    ...
    async def setnotifchannel_slash(
        ...
        await db.update_guild_config(
            interaction.guild.id, {"notif_channel": channel.id}
        )
        await interaction.response.send_message(
            embed=UI.success(
                interaction.user, f"Notification channel set to {channel.mention}."
            ),
            ephemeral=True,
        )
    ...
    async def setnotifchannel_prefix(
        ...
        await ctx.reply(
            embed=UI.success(
                ctx.author, f"Notification channel set to {channel.mention}."
            )
        )
    ...
    @app_commands.command(
        name="setnofifrole",
        description="Set the role to mention in announcements. Admin only.",
    )
    ...
    async def setnofifrole_slash(
        ...
        await interaction.response.send_message(
            embed=UI.success(
                interaction.user, f"Notification role set to {role.mention}."
            ),
            ephemeral=True,
        )
    ...
    async def setnofifrole_prefix(
        ...
        await ctx.reply(
            embed=UI.success(ctx.author, f"Notification role set to {role.mention}.")
        )
    ...
    @app_commands.command(
        name="earnsettings",
        description="Configure which earn commands are enabled. Admin only.",
    )
    @app_commands.checks.has_permissions(administrator=True)
    ...
    @app_commands.command(
        name="denkireport",
        description="Report a user to the bot owner for review. Admin only.",
    )
    ...
    async def _denkireport(
        ...
        await _respond(
            ...
            UI.success(
                author,
                f"Report filed against **{target.display_name}**. The bot owner has been notified.",
            ),
            is_slash,
            ephemeral=True,
        )
...
class ConfigView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Earn settings", style=discord.ButtonStyle.primary, emoji=E_GEAR
    )
    async def earn_settings(
...
class EarnSettingsView(discord.ui.View):
    ...
    async def select_disabled(
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=UI.error(interaction.user, str(e)), ephemeral=True
            )
            return

        enabled_names = [
            k.replace("_enabled", "").capitalize() for k, v in updates.items() if v
        ]
        disabled_names = [
            k.replace("_enabled", "").capitalize() for k, v in updates.items() if not v
        ]
        ...
        await interaction.response.send_message(
            embed=UI.success(
                interaction.user, "Earn settings updated.\n" + "\n".join(lines)
            ),
            ephemeral=True,
        )
        self.stop()
