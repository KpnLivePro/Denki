    ...
    async def convert(self, ctx: commands.Context[Any], argument: str) -> int:
        ...
        try:
            ...
            raise commands.BadArgument(
                f"`{argument}` is not a valid amount. Examples: `6000`, `1.5k`, `-500`"
            )
...
class TablePaginatorView(PaginatorView):
    ...
    async def _rebuild_pages(self) -> list[discord.Embed]:
        try:
            ...
            return [UI.base(f"> `❗` *Refresh failed: {e}*")]
...
class Sudo(commands.Cog):
    ...
    async def warn(self, ctx: commands.Context[Any], user_id: Annotated[int, UserID()], *, reason: str) -> None:
        ...
        try:
            await target.send(embed=UI.warn_dm(reason=reason, warn_count=warn_count))
        except discord.Forbidden:
            pass
        except Exception as e:
            logger.error("Error DMing warn to %d: %s", user_id, e)

        await _respond(ctx, UI.warn_issued(user=target, reason=reason, warn_count=warn_count))
    ...
    async def warns(self, ctx: commands.Context[Any], user_id: Annotated[int, UserID()]) -> None:
        ...
        if not warns_list:
            return await _respond(ctx, UI.info(ctx.author, f"**{username}** has no active warns."))

        embed = UI.base(f"> `⚠️` *Active warns for **{username}** ({len(warns_list)} / 3)*")
        for w in warns_list:
            embed.add_field(
    ...
    async def _execute_ban(self, ctx: commands.Context[Any], user_id: int, reason: str, silent: bool) -> None:
        ...
        try:
            target = await self.bot.fetch_user(user_id)
            await target.send(embed=UI.ban_dm(reason=reason))
        except discord.Forbidden:
            pass
        except Exception as e:
            logger.error("Error DMing ban to %d: %s", user_id, e)
    ...
    async def unban(self, ctx: commands.Context[Any], user_id: Annotated[int, UserID()]) -> None:
        ...
        if not ban:
            return await _respond(ctx, UI.error(ctx.author, f"User `{user_id}` is not currently banned."))
        await db.unban_user(user_id)
        try:
            username = str(await self.bot.fetch_user(user_id))
        except Exception:
            username = f"User {user_id}"
        await _respond(ctx, UI.success(ctx.author, f"**{username}** has been unbanned from Denki."))
    ...
    async def wallet(self, ctx: commands.Context[Any], user_id: Annotated[int, UserID()]) -> None:
        ...
        if not user_data:
            return await _respond(ctx, UI.error(ctx.author, f"User `{user_id}` has no wallet record."))
        try:
            username = str(await self.bot.fetch_user(user_id))
        except Exception:
            username = f"User {user_id}"
    ...
    async def seasonend(self, ctx: commands.Context[Any]) -> None:
        ...
        if not season:
            return await _respond(ctx, UI.error(ctx.author, "There is no active season to end."))
        ...
        if view.confirmed:
            ...
            await ctx.reply(embed=UI.success(ctx.author, "Season end logic completed successfully."))
        else:
            await ctx.reply(embed=UI.info(ctx.author, "Season end cancelled."))
    ...
    async def seasonset(self, ctx: commands.Context[Any], *, args: str) -> None:
        ...
        season = await db.get_active_season()
        if not season:
            return await _respond(ctx, UI.error(ctx.author, "There is no active season."))

        parts = args.strip().split()
        if not parts:
            return await _respond(ctx, UI.error(ctx.author, "Provide a season name."))
        ...
        if not parts:
            return await _respond(ctx, UI.error(ctx.author, "Provide a season name before the color."))
    ...
    async def announce(self, ctx: commands.Context[Any], *, message: str) -> None:
        try:
            ...
            return await _respond(ctx, UI.error(ctx.author, f"Failed to fetch guild configs: {e}"))

        if not configs:
            return await _respond(ctx, UI.error(ctx.author, "No servers have a notification channel configured."))
    ...
    async def reports(self, ctx: commands.Context[Any]) -> None:
        ...
        if not pending:
            return await _respond(ctx, UI.info(ctx.author, "No pending reports."))
        embed = UI.base(f"> `📋` *Pending reports ({len(pending)})*")
        for r in pending[:10]:
            embed.add_field(
    ...
    async def dismiss(self, ctx: commands.Context[Any], report_id: int) -> None:
        ...
        await _respond(ctx, UI.success(ctx.author, f"Report `{report_id}` dismissed."))
    ...
    async def presence(self, ctx: commands.Context[Any], activity_type: str, *, text: str = "") -> None:
        ...
        if activity_type == "clear":
            ...
            return await _respond(ctx, UI.success(ctx.author, "Activity cleared."))
        ...
        if activity_type not in type_map:
            ...
            return await _respond(ctx, UI.error(ctx.author, f"Invalid type. Choose: {valid}, `clear`"))
        if not text:
            return await _respond(ctx, UI.error(ctx.author, "Provide text after the activity type."))
        ...
        await _respond(ctx, UI.success(ctx.author, f"Activity set to **{activity_type}** `{text}`."))
    ...
    async def status(self, ctx: commands.Context[Any], status_val: str) -> None:
        ...
        if status_val.lower() not in status_map:
            ...
            return await _respond(ctx, UI.error(ctx.author, f"Invalid status. Choose: {valid}"))
        await self.bot.change_presence(status=status_map[status_val.lower()])
        await _respond(ctx, UI.success(ctx.author, f"Status set to `{status_val.lower()}`."))
    ...
    async def botstatus(self, ctx: commands.Context[Any], *, text: str) -> None:
        if text.lower() == "clear":
            ...
            return await _respond(ctx, UI.success(ctx.author, "Custom status cleared."))
        await self.bot.change_presence(activity=discord.CustomActivity(name=text))
        await _respond(ctx, UI.success(ctx.author, f"Custom status set to `{text}`."))
    ...
    async def sys_stats(self, ctx: commands.Context[Any]) -> None:
        ...
        try:
            import resource
            mem_str = f"{resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024:.1f} MB"
        except Exception:
            mem_str = "N/A"
    ...
    async def botctl(self, ctx: commands.Context[Any]) -> None:
        await _respond(ctx, UI.info(ctx.author, "Usage: `!d bot restart`"))

    @botctl.command(name="restart")
    async def botctl_restart(self, ctx: commands.Context[Any]) -> None:
        view = ConfirmView(ctx.author.id)
        await ctx.reply(
            embed=UI.warn(ctx.author, "Restart the bot? The bot will be offline for a few seconds."),
            view=view,
        )
        await view.wait()
        if not view.confirmed:
            await ctx.reply(embed=UI.info(ctx.author, "Restart cancelled."))
            return
        await ctx.reply(embed=UI.success(ctx.author, "Restarting..."))
    ...
    async def cogs_group(self, ctx: commands.Context[Any]) -> None:
        ...
        await _respond(ctx, UI.base(f"> `🧩` *Loaded cogs ({len(loaded)})*\n\n{lines}"))
    ...
    async def cogs_reload(self, ctx: commands.Context[Any], cog: str = "all") -> None:
        ...
        if cog.lower() == "all":
            ...
            await _respond(ctx, UI.base("> `🔄` *Reloaded all cogs*\n\n" + "\n".join(results)))
        else:
            ext = cog if "." in cog else f"cogs.{cog}"
            try:
                await self.bot.reload_extension(ext)
                await _respond(ctx, UI.success(ctx.author, f"Reloaded `{ext}`."))
            except commands.ExtensionNotLoaded:
                await _respond(ctx, UI.error(ctx.author, f"`{ext}` is not loaded."))
            except commands.ExtensionNotFound:
                await _respond(ctx, UI.error(ctx.author, f"`{ext}` not found."))
            except Exception as e:
                await _respond(ctx, UI.error(ctx.author, f"Failed to reload `{ext}`:\n```{e}```"))
    ...
    async def data_cmd(self, ctx: commands.Context[Any], table: str | None = None) -> None:
        ...
        if table is None:
            ...
            embed  = UI.base(f"> `🗄️` *DB Health — {status}*\n\n{lines2}")
        ...
        if table not in valid_tables:
            ...
            return await _respond(ctx, UI.error(ctx.author, f"Unknown table. Valid: {valid}"))

        try:
            res  = db.supabase.table(table).select("*").limit(50).execute()
            rows: list[Any] = res.data or []
        except Exception as e:
            return await _respond(ctx, UI.error(ctx.author, f"Query failed: {e}"))
