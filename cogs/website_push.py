...
Required environment variables:
  WEBSITE_API_URL  — full URL to POST to e.g. https://kww-api.azurewebsites.net/api/denki/push
  WEBSITE_SECRET   — shared secret checked by the Flask API

Auth: secret is sent both as X-API-Secret header AND as a JSON body field
so the Flask endpoint can accept either form.
"""
...
class WebsitePush(commands.Cog):
    ...
    async def cog_load(self) -> None:
        ...
        logger.info(
            "Website push loop started — interval=%ds url=%s",
            PUSH_INTERVAL, WEBSITE_API_URL,
        )
    ...
    async def _build_payload(self) -> dict[str, Any]:
        ...
        return {
            # Secret in body — Flask reads this if no header present
    ...
    async def _post(self, payload: dict[str, Any]) -> None:
        # Send secret in BOTH the JSON body and the header so the Flask
        # endpoint can authenticate regardless of which it checks first.
        headers = {
            "Content-Type":  "application/json",
            "X-API-Secret":  WEBSITE_SECRET,
        }
