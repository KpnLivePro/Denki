...
class Economy(commands.Cog):
    ...
    async def _daily(self, ctx_or_interaction: Any, is_slash: bool) -> None:
        ...
        if not config["daily_enabled"]:
            return await _respond(
                ctx_or_interaction,
                UI.error("The `/daily` command is disabled in this server."),
                is_slash,
            )
        ...
        if last:
            ...
            if elapsed < DAILY_COOLDOWN:
                return await _respond(
                    ctx_or_interaction,
                    UI.cooldown("daily", format_remaining(DAILY_COOLDOWN - elapsed)),
                    is_slash,
                )
        ...
        await _respond(
            ctx_or_interaction,
            UI.daily(user=author, amount=amount, wallet=int(wallet_data["wallet"]), tier=tier),
            is_slash,
        )
    ...
    async def _work(self, ctx_or_interaction: Any, is_slash: bool) -> None:
        ...
        if not config["work_enabled"]:
            return await _respond(
                ctx_or_interaction,
                UI.error("The `/work` command is disabled in this server."),
                is_slash,
            )
        ...
        if last:
            ...
            if elapsed < WORK_COOLDOWN:
                return await _respond(
                    ctx_or_interaction,
                    UI.cooldown("work", format_remaining(WORK_COOLDOWN - elapsed)),
                    is_slash,
                )
        ...
        await _respond(
            ctx_or_interaction,
            UI.work(user=author, job=job, amount=amount, wallet=int(wallet_data["wallet"])),
            is_slash,
        )
    ...
    async def _rob(self, ctx_or_interaction: Any, target: discord.Member, is_slash: bool) -> None:
        ...
        if target.id == author.id:
            return await _respond(ctx_or_interaction, UI.error("You can't rob yourself."), is_slash)
        if target.bot:
            return await _respond(ctx_or_interaction, UI.error("You can't rob bots."), is_slash)

        config = await db.get_or_create_guild_config(guild_id)
        if not config["rob_enabled"]:
            return await _respond(
                ctx_or_interaction,
                UI.error("The `/rob` command is disabled in this server."),
                is_slash,
            )
        ...
        if last:
            ...
            if elapsed < ROB_COOLDOWN:
                return await _respond(
                    ctx_or_interaction,
                    UI.cooldown("rob", format_remaining(ROB_COOLDOWN - elapsed)),
                    is_slash,
                )
        ...
        if victim_wallet < ROB_MIN_VICTIM:
            return await _respond(
                ctx_or_interaction,
                UI.error(f"{target.display_name} doesn't have enough ¥ Yen to rob."),
                is_slash,
            )
    ...
    async def _pay(
        ...
        if target.id == author.id:
            return await _respond(ctx_or_interaction, UI.error("You can't pay yourself."), is_slash)
        if target.bot:
            return await _respond(ctx_or_interaction, UI.error("You can't pay bots."), is_slash)
        ...
        if amount_str.lower() == "all":
            ...
            try:
                ...
                return await _respond(
                    ctx_or_interaction,
                    UI.error("Invalid amount. Enter a number or `all`."),
                    is_slash,
                )
    ...
    async def _vote(self, ctx_or_interaction: Any, is_slash: bool) -> None:
        ...
        if not topgg_token:
            return await _respond(
                ctx_or_interaction,
                UI.error("Vote rewards are not configured yet. Check back soon!"),
                is_slash,
            )
        ...
        if not result["voted"]:
            ...
            return await _respond(
                ctx_or_interaction,
                UI.vote_prompt(vote_url=TOPGG_VOTE_URL, current_streak=streak),
                is_slash,
            )
